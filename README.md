...
> **Why Per-Vendor Translation Instead of a Canonical Model?**  
A common question when designing multi-vendor automation is whether to introduce an intermediate canonical model between the Intent and Translation layers. In such an approach, InterfaceIntent would first translate to a vendor-neutral CanonicalInterface representation, which would then be adapted to each vendor's specific YANG model. While this sounds appealing in theory, it creates redundant abstraction in practice.
The critical insight is that OpenConfig and IETF YANG models already serve as canonical representations. When both SR-Linux and cEOS support openconfig-interfaces, the data structure is identical — only the serialization format (XML vs JSON) differs. Creating a second canonical layer on top of OpenConfig means translating from one canonical model to another, which provides no additional abstraction value. Furthermore, production networks require vendor-specific features (VRFs, policy routing, advanced QoS) that exist only in native YANG models. A canonical model either becomes bloated by including every vendor's extensions, or it supports only the lowest common denominator and forces engineers to bypass the abstraction for real-world features. The Intent layer already provides vendor-agnostic business semantics; the Translation layer leverages YANG as the technical canonical model. This two-layer approach is both simpler and more powerful than inserting a third layer between them.
Canonical models do have legitimate use cases in network automation: normalizing operational state for monitoring dashboards, defining behavioral contracts for cross-vendor testing, and building vendor-agnostic network data warehouses. But for configuration generation — the focus of this course — per-vendor translation with shared OpenConfig models where available is the right architectural choice. It keeps abstraction where it adds value (the Intent layer) and avoids abstraction where it adds complexity (a redundant canonical model).
> 
